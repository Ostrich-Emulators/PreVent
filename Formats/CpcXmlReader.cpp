...
const std::string CpcXmlReader::MISSING_VALUESTR( "-32768" );
const std::set<std::string> CpcXmlReader::Hz60({ "RR", "VNT_PRES", "VNT_FLOW" } );
const std::set<std::string> CpcXmlReader::Hz120({ "ICP1", "ICP2", "ICP4", "LA4" } );

CpcXmlReader::CpcXmlReader( ) : reader( 0 ), warnMissingName( true ), warnJunkData( true ),
prevtime( 0 ), currvstime( 0 ), lastvstime( 0 ), currwavetime( 0 ), lastwavetime( 0 ) {
  LIBXML_TEST_VERSION
}

CpcXmlReader::CpcXmlReader( const CpcXmlReader& orig ) {
}

CpcXmlReader::~CpcXmlReader( ) {
}

void CpcXmlReader::finish( ) {
  if ( NULL == reader ) {
...
size_t CpcXmlReader::getSize( const std::string& input ) const {
  struct stat info;
...
int CpcXmlReader::prepare( const std::string& input, SignalSet& info ) {
...
ReadResult CpcXmlReader::fill( SignalSet & info, const ReadResult& lastfill ) {
...
ReadResult CpcXmlReader::processNode( SignalSet& info ) {
  int ret = next( );
  if ( ret < 0 ) {
...
ReadResult CpcXmlReader::handleSegmentPatientName( SignalSet& info ) {
...
bool CpcXmlReader::isRollover( bool forVitals ) {
...
void CpcXmlReader::handleWaveformSet( SignalSet& info ) {
  next( );
  std::string element = stringAndFree( xmlTextReaderName( reader ) );
...
void CpcXmlReader::handleVitalsSet( SignalSet & info ) {
  next( );
  std::string element = stringAndFree( xmlTextReaderName( reader ) );
  while ( "VS" == element ) {
    std::string param;
...
DataRow CpcXmlReader::handleOneVs( std::string& param, std::string& uom ) {
...
std::map<std::string, std::string> CpcXmlReader::getHeaders( ) {
  std::map<std::string, std::string> map;
...
std::map<std::string, std::string> CpcXmlReader::getAttrs( ) {
  std::map<std::string, std::string> map;
...
std::string CpcXmlReader::trim( std::string & totrim ) const {
  // ltrim
...
std::string CpcXmlReader::textAndClose( ) {
  std::string ret = text( );
  int nodetype = xmlTextReaderNodeType( reader );
...
std::string CpcXmlReader::textAndAttrsToClose( std::map<std::string, std::string>& attrs ) {
...
std::string CpcXmlReader::stringAndFree( xmlChar * chars ) const {
  std::string ret;
  if ( NULL != chars ) {
...
std::string CpcXmlReader::nextelement( ) {
  // ignore everything until the next start element
...
std::string CpcXmlReader::resample( const std::string& data, int hz ) {
  /**
...
bool CpcXmlReader::waveIsOk( const std::string& wavedata ) {
